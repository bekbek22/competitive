...
int main() {
    ...
    // Check if n is greater than the number of words in the input
    int num_words = 0;
    string word;
    while (cin >> word) {
        num_words++;
    ...
    // Check if ans is a valid integer
    bool ans_is_int = true;
    try {
        stoi(ans);
    } catch (...) {
        ans_is_int = false;
    ...
    if (ans_is_int) {
        // Read the input words and check if they match the given number
        cin.clear();
        cin.seekg(0);
        while (n--) {
            string word;
